...
## H1
```{r}
lm1_1 <- glm(voted ~ swd_pre + age + I(age^2) + female +
                 postsecondary_edu + factor(election), 
             data = all_panels)
...
tab_model(list(lm1_1, lm1_2, lm1_3, lm1_4))
```

## H2
```{r}
lm2_1 <- glm(voted ~ swd_pre + age + I(age^2) + female +
                 postsecondary_edu + factor(election), 
             data = all_panels)
...
tab_model(list(lm3_1, lm3_2, lm3_3))
```
...
cz_lm1_2 <- glm(voted ~ swd_pre + age + I(age^2) + female + 
                 postsecondary_edu +
                 pol_knowledge + party_close, data = cz_1996)
