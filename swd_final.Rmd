...
swd3_round1 <- lm_robust(swd_diff ~ voter_type + 
                           female + age +
                    postsecondary_edu + pol_interest_num + 
                    pol_knowledge + party_close + duty_to_vote, 
                  data = cz_2023_all_waves_resp, 
...
swd4_round1 <- lm_robust(swd_diff ~ prez_vote_type + 
                    female + age +
                      postsecondary_edu + pol_interest_num + 
                      pol_knowledge + party_close + duty_to_vote, 
                  data = cz_2023_all_waves_resp, 
...
swd5_round1 <- lm_robust(swd_diff ~ stable_voter + 
                           female + age +
                    postsecondary_edu + pol_interest_num + 
                    pol_knowledge + party_close + duty_to_vote, 
                  data = pavel_voters, 
                  weights = weight_turnout, 
                  se_type = "stata")
swd5_round1_ord <- lm_robust(swd_diff ~ pavel_decision + 
                        female + age +
                    postsecondary_edu + pol_interest_num + 
                    pol_knowledge + party_close + duty_to_vote, 
                  data = pavel_voters, 
...
```{r}
ce1b <- lm_robust(swd_diff ~ voted + female + age +
             postsecondary_edu + 
             pol_interest_num + party_close + 
             election, data = all_panels_both_waves, 
           weights = weight_turnout, 
           se_type = "stata") 
ce2b <- lm_robust(swd_diff ~ voter_type + female + age +
             postsecondary_edu + 
             pol_interest_num + party_close + 
             election, data = all_panels_both_waves, 
           weights = weight_turnout, 
...
modelsummary::modelsummary(
  ...
  coef_map = c(
    ...
    "winner_stable2full_winner:wave2" = "Full winner (A) × Wave 2", 
    "winner_stable2full_winner:wave3" = "Full winner (A) × Wave 3",
    "winner_stable2full_winner:wave4" = "Full winner (A) × Wave 4", 
    "winner_stable2full_winner:wave5" = "Full winner (A) × Wave 5",
...
## Robust Table 4: SWD change, CZ 2023 panel
```{r}
pavel_voters <- cz_2023_all_waves_resp %>% 
  filter(PRES2023CAND2r_w4 == "Petr Pavel")

swd1_round1 <- lm_robust(swd_diff ~ voted + swd_pre + female + 
                           age +
                    postsecondary_edu + pol_interest_num + 
                    pol_knowledge + party_close + duty_to_vote, 
             data = cz_2023_all_waves_resp, 
...
swd1_round1b <- lm_robust(swd_diff ~ voted + swd_pre + female + 
                            age +
                    postsecondary_edu + pol_interest_num + 
                    pol_knowledge + party_close + duty_to_vote, 
             data = cz_2023_all_waves_resp, 
...
modelsummary(list(swd1_round1, swd3_round1, 
             ...
             gof_map = c("nobs", "r.squared", "adj.r.squared"),
             # notes = c("Data weighted by turnout."),
             output = "output/app_tab4_models_robust.tex")
...
knitr::kable(cze, digits = 2, 
             col.names = c("Variable", "mean", 
                           ...
                           "max", "N"), 
             format = "latex") %>% 
  writeLines(., "output/app_tab2_descriptives_cz2023.tex")
```
...
knitr::kable(total, digits = 2, format = "latex", 
             col.names = c("Variable", "mean", 
                           "SD", "min", "Q1", "median", "Q3", 
                           "max", "N")) %>% 
  writeLines(., "output/app_tab1_descriptives_total.tex")
